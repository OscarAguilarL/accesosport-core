...
@AllArgsConstructor
public class GetEventRegistrationsPagedUseCase extends UseCase<GetEventRegistrationsPagedUseCase.PagedCommand, PageResult<ParticipantInEventResponse>> {

    public record PagedCommand(UUID eventId, PageQuery query) {}

    private final RegistrationRepository registrationRepository;
    private final ParticipantProfileRepository participantProfileRepository;

    @Override
    protected PageResult<ParticipantInEventResponse> internalExecute(PagedCommand command) {
        PageResult<Registration> page = registrationRepository.findByEventId(command.eventId(), command.query());
        List<ParticipantInEventResponse> content = page.content().stream()
                .map(this::toResponse)
                .toList();
        return PageResult.of(content, page.page(), page.size(), page.totalElements());
    }
